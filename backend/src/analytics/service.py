...
def collect_event(
    ...
    *,
    ...
    precise_location_consent = bool(payload.consent_granted)

    # Treat latitude and longitude as one atomic pair. This prevents combining
    # one browser coordinate with one IP-derived coordinate.
    has_browser_coordinates = (
        precise_location_consent
        and payload.latitude is not None
        and payload.longitude is not None
    )
    has_ip_coordinates = (
        identity.latitude is not None
    ...
    *,
    ...
    return {
        ...
        "top_clicks": _dimension(
            ...
            AnalyticsEvent.target_key,
            filters=(
                AnalyticsEvent.event_type == "navigation_click",
                AnalyticsEvent.target_key.is_not(None),
            ),
            limit=30,
        ),
